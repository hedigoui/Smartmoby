...
public class GoogleCalendar {
    ...
    public String recupererEventId(String title) throws IOException, GeneralSecurityException {
        ...
        List<Event> items = events.getItems();
        for (Event event : items) {
            // Log the event title for debugging
            System.out.println("Événement trouvé : " + event.getSummary());

            if (event.getSummary() != null && event.getSummary().equalsIgnoreCase(title)) {
                return event.getId(); // Return the event ID if the title matches
            }
        }
